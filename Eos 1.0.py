...
# print name of virtual assistant(VA)
print('Eos 1.0')

# setting up Speech Engine using  API 'sapi5' and setting it to male voice
engine = pyttsx3.init('sapi5')
voices = engine.getProperty('voices')
engine.setProperty('voice', 'voices[0].id')  # 0 is for male voice, 1 is for female voice


# setting up VA to speak the command and then wait
# defining function speak
def speak(text):
    engine.say(text)
    engine.runAndWait()


# Initiate a function to greet the user according to time
# defining function wishMe
def wishMe():
    hour = datetime.datetime.now().hour
    # if time is in between 12 am and 12 pm, then wish good morning
    if hour >= 0 and hour < 12:
        speak("Good Morning")
        print("Good Morning")
    # if time is in between 12 pm and 6 pm, then wish good afternoon
    elif hour >= 12 and hour < 18:
        speak("Good Afternoon")
        print("Good Afternoon")
    # if time is in between 6 pm and 12 am, then wish good evening
    else:
        speak("Good Evening")
        print("Good Evening")


# setting up the microphone to receive user's commands as input and that audio will be the user's statement
# defining takecommand function for AI to understand and to accept human language.
def takeCommand():
    r = sr.Recognizer()
    # defining microphone's input as source of audio
    with sr.Microphone() as source:
        print("Listening...")
        audio = r.listen(source)

        try:
            # defining variable statement as recognition of audio
            statement = r.recognize_google(audio, language='en-in')
            # print inputed audio as text
            print(f"user said:{statement}\n")
            # if the VA doesn't recognise what's said it raises an exception indicating the user to repeat his/her command
...
# setting up the VA to indicate it's presence and wish the user
# first sentence uttered by VA
speak("Loading your AI personal assistant Eos")
wishMe()  # Greeting

# indicates that the module is being run by itself and is used to define a standalone module that has been imported by python for this code
# initiates the main function, all the commands are stored in variable statement
if __name__ == '__main__':

    # while the code is true all the commands will run, while while loop ensures that the statement how may I be of service will be repeated
    while True:
        speak("How may I be of service?")  # the second command after wishing the user, which starts the program running
        statement = takeCommand().lower()  # the response to the speak command, which tells the program what to run
        if statement == 0:
            continue
        # when done with the program these commands will make the program sign out
        if "good bye" in statement or "ok bye" in statement or "stop" in statement or "bye" in statement \
                or "tata" in statement:
            speak('I shall see you later')  # speak command
            print('I shall see you later')
            break  # breaks loop
        # this command also signs out the VA
        if 'thank you' in statement:
            ...
            if 'no' in statement:
                ...
                break
        # this command will search wikipedia for the search terms the user says and will return a summary of the results
        ...
        # opens google
            ...
            time.sleep(5)
        # tells the weather according to the city specified
        ...
        # answers the question who made you
        ...
        # opens up the latest news headlines from the hindu
        ...
        # answers basic computational and geographical questions
        elif 'ask' in statement or 'what is' in statement:
            speak('I can answer to computational and geographical questions and what question do you want to ask now')
            question = takeCommand()
            # wolfram's app ID which is unique to each person and can be used to interact with the app
        ...
        # thsi command switches off the computer using subprocess
        elif "log off" in statement or "sign out" in statement:
            speak("Ok , your pc will log off in 10 sec make sure you exit from all applications")
            subprocess.call(["shutdown", "/l"])
        # this entails all the webautomation and interaction with web elements in CLiC's website
            ...
            if 'concordance' in statement or 'khan' in statement or 'confidence' in statement \
                ...
                # html locates the web element using it's xpath
                CONCORDANCE_BUTTON_XPATH = '//*[@id="control-bar"]/div/fieldset[1]/legend/a'  # the html (xpath for the element concordance)
                browser = driver
                button = browser.find_element(By.XPATH, CONCORDANCE_BUTTON_XPATH)
                button.click()  # opens concordance tab
                time.sleep(1)
                ...
                SEARCH_CORPORA_BUTTON_XPATH = '//*[@id="control-bar"]/div/fieldset[1]/form/div[1]/ul/li/input'  # the html (xpath for the element concordance)
                browser = driver
                button = browser.find_element(By.XPATH, SEARCH_CORPORA_BUTTON_XPATH)
                button.click()  # clicks in the search bar
                ...
                statement = takeCommand().lower()
                # clicks on the option which is heard in the user's commands
                ...
                # if the book was not found
                    ...
                    statement = takeCommand().lower()
                # clicks on the only in subsets search bar
                ...
                statement = takeCommand().lower()
                # listens to the user's commands and clicks on the option that matches the words in the audio
                if 'all text' in statement:
                    el1 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[1]/form/div[2]/a/span')
                    for option in el1.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[1]/form/div[2]/div/ul/li[1]'):
                        option.click()
                if 'short suspensions' in statement:
                    el2 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[1]/form/div[2]/a/span')
                    for option in el2.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[1]/form/div[2]/div/ul/li[2]'):
                        option.click()
                if 'long suspensions' in statement:
                    el3 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[1]/form/div[2]/a/span')
                    for option in el3.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[1]/form/div[2]/div/ul/li[3]'):
                        option.click()
                if 'quotes' in statement:
                    el4 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[1]/form/div[2]/a/span')
                    for option in el4.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[1]/form/div[2]/div/ul/li[4]'):
                        option.click()
                if 'non quotes' in statement:
                    el5 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[1]/form/div[2]/a/span')
                    for option in el5.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[1]/form/div[2]/div/ul/li[5]'):
                        option.click()
                # clicks on the textbox for search terms for the user to type
                ...
                for option in TEXTBOX_TERMS.find_elements(By.XPATH,
                                                          '//*[@id="control-bar"]/div/fieldset[1]/form/input[1]'):
                    option.click()
                time.sleep(5)
                # asks the user which option he'she prefers then generates the results
                speak("Are you looking for a whole phrase or any word?")
                statement = takeCommand().lower()
                if "whole phrase" in statement or 'phrase' in statement:
                    W_P = driver.find_element(By.XPATH,
                                              '//*[@id="control-bar"]/div/fieldset[1]/form/div[3]/label[1]/input')
                    for option in W_P.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[1]/form/div[3]/label[1]/input'):
                        option.click()
                else:
                    A_W = driver.find_element(By.XPATH,
                                              '//*[@id="control-bar"]/div/fieldset[1]/form/div[3]/label[2]/input')
                    for option in A_W.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[1]/form/div[3]/label[2]/input'):
                        option.click()
                press('enter')  # generates the results
                speak("Here are your results")
                # asks the user how he/she wants the results displayed
                speak(
                    "do you want to view the results as 'basic results', as 'full metadata', or as a 'distribution plot' ")
                statement = takeCommand().lower()
                if 'basic results' in statement or 'results' in statement:
                    ...
                    for option in B_R.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[1]/form/label[5]/input'):
                        option.click()
                if 'full metadata' in statement or 'metadata' in statement:
                    ...
                    for option in F_M.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[1]/form/label[6]/input'):
                        option.click()
                if 'distribution plot' in statement or 'plot' in statement:
                    ...
                    for option in D_P.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[1]/form/label[7]/input'):
                        option.click()
                # asks if the user would like to filter rows and clicks on search box for user to type in termsn ans hit enter
                ...
                for option in S_T.find_elements(By.XPATH,
                                                '//*[@id="control-bar"]/div/fieldset[1]/form/div[5]/ul/li/input'):
                    option.click()
                    time.sleep(5)
            ...
            if 'subsets' in statement or 'subset' in statement:
                ...
                statement = takeCommand().lower()
                SEARCH2_CORPORA_BUTTON_XPATH = '//*[@id="control-bar"]/div/fieldset[2]/form/div[1]/ul'  # clicks on corpora options textbox
                browser = driver
                ...
                speak("The Corpora options are Dickens Novels, 19th Century Reference Corpus, Children's Literature, "
                      "Additional Requested Texts and African American Writers ")
                statement = takeCommand().lower()
                # selects the option according to voice commands
                if 'Dickens Novels' in statement or 'Decans Novels' in statement \
                        or 'novels' in statement:
                    ...
                    speak("How else may I be of service?")
                    statement = takeCommand().lower()
                # selects the search within subsets textbox
                ...
                print("The subsets available are displayed. What type of text are you looking for?")
                statement = takeCommand().lower()
                # selects the options according to voice command
                if 'short suspensions' in statement or 'short' in statement:
                    sue1 = driver.find_element(By.XPATH,
                                               '//*[@id="control-bar"]/div/fieldset[2]/form/div[2]/div/ul/li[1]')
                    for option in sue1.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[2]/form/div[2]/div/ul/li[1]'):
                        option.click()
                if 'long suspensions' in statement or 'long' in statement:
                    sue3 = driver.find_element(By.XPATH,
                                               '//*[@id="control-bar"]/div/fieldset[2]/form/div[2]/div/ul/li[2]')
                    for option in sue3.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[2]/form/div[2]/div/ul/li[2]'):
                        option.click()
                if 'quotes' in statement or 'quote' in statement or 'hote' in statement:
                    sue4 = driver.find_element(By.XPATH,
                                               '//*[@id="control-bar"]/div/fieldset[2]/form/div[2]/div/ul/li[3]')
                    for option in sue4.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[2]/form/div[2]/div/ul/li[3]'):
                        option.click()
                if 'non quotes' in statement or 'non' in statement:
                    sue5 = driver.find_element(By.XPATH,
                                               '//*[@id="control-bar"]/div/fieldset[2]/form/div[2]/div/ul/li[4]')
                    for option in sue5.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[2]/form/div[2]/div/ul/li[4]'):
                        option.click()
                press('enter')  # generates results
                speak("Here are your results")
                # asks how the user would want to view his/her results
                speak(
                    "do you want to view the results as 'basic results', as 'full metadata', or as a 'distribution plot' ")
                statement = takeCommand().lower()
                if 'basic results' in statement or 'results' in statement:
                    BA_R = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[2]/form/label[4]/input')
                    for option in BA_R.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[2]/form/label[4]/input'):
                        option.click()
                if 'full metadata' in statement or 'metadata' in statement:
                    FA_M = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[2]/form/label[5]/input')
                    for option in FA_M.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[2]/form/label[5]/input'):
                        option.click()
                if 'distribution plot' in statement or 'plot' in statement:
                    DA_P = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[2]/form/label[6]/input')
                    for option in DA_P.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[2]/form/label[6]/input'):
                        option.click()
                speak("would you like to filter any rows?")  # asks the users if they'd like to filter results
                statement = takeCommand().lower()
                if 'yes' in statement:
                    speak('please type in your filter terms')
                ...
                for option in SA_T.find_elements(By.XPATH,
                                                 '//*[@id="control-bar"]/div/fieldset[2]/form/div[6]/ul/li/input'):
                    option.click()
                    time.sleep(5)
                press('enter')  # generates results
            ...
            if 'clusters' in statement or 'cluster' in statement:
                ...
                SEARCH3_CORPORA_BUTTON_XPATH = '//*[@id="control-bar"]/div/fieldset[3]/form/div[1]/ul'  # clicks on searchbox for corpora
                browser = driver
                button = browser.find_element(By.XPATH, SEARCH3_CORPORA_BUTTON_XPATH)
                button.click()
                # selects the option according to option in statement
                speak("The Corpora options are Dickens Novels, 19th Century Reference Corpus, Children's Literature, "
                      "Additional Requested Texts and African American Writers ")
                statement = takeCommand().lower()
                if 'Dickens Novels' in statement or 'Decans Novels' in statement \
                    ...
                    speak("How else may I be of service?")
                    statement = takeCommand().lower()
                # opens the searchbox for subsets and clic son the option according to voice command
                ...
                statement = takeCommand().lower()
                if 'all text' in statement:
                    boo1 = driver.find_element(By.XPATH,
                                               '//*[@id="control-bar"]/div/fieldset[3]/form/div[2]/div/ul/li[1]')
                    for option in boo1.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[3]/form/div[2]/div/ul/li[1]'):
                        option.click()
                if 'short suspensions' in statement or 'short' in statement:
                    boo2 = driver.find_element(By.XPATH,
                                               '//*[@id="control-bar"]/div/fieldset[3]/form/div[2]/div/ul/li[2]')
                    for option in boo2.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[3]/form/div[2]/div/ul/li[2]'):
                        option.click()
                if 'long suspensions' in statement or 'long' in statement:
                    boo3 = driver.find_element(By.XPATH,
                                               '//*[@id="control-bar"]/div/fieldset[3]/form/div[2]/div/ul/li[3]')
                    for option in boo3.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[3]/form/div[2]/div/ul/li[3]'):
                        option.click()
                if 'quotes' in statement:
                    boo4 = driver.find_element(By.XPATH,
                                               '//*[@id="control-bar"]/div/fieldset[3]/form/div[2]/div/ul/li[4]')
                    for option in boo4.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[3]/form/div[2]/div/ul/li[4]'):
                        option.click()
                if 'non quotes' in statement or 'non' in statement:
                    boo5 = driver.find_element(By.XPATH,
                                               '//*[@id="control-bar"]/div/fieldset[3]/form/div[2]/div/ul/li[5]')
                    for option in boo5.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[3]/form/div[2]/div/ul/li[5]'):
                        option.click()
                        # selects the ngram
                ...
                g0 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[3]/form/div[3]/div/div/input')
                for option in g0.find_elements(By.XPATH,
                                               '//*[@id="control-bar"]/div/fieldset[3]/form/div[3]/div/div/input'):
                    option.click()
                statement = takeCommand().lower()
                if '1 gram' in statement or 'one' in statement:
                    g1 = driver.find_element(By.XPATH,
                                             '//*[@id="control-bar"]/div/fieldset[3]/form/div[3]/div/ul/li[1]')
                    for option in g1.find_elements(By.XPATH,
                                                   '//*[@id="control-bar"]/div/fieldset[3]/form/div[3]/div/ul/li[1]'):
                        option.click()
                if '2 gram' in statement or 'two' in statement:
                    g2 = driver.find_element(By.XPATH,
                                             '//*[@id="control-bar"]/div/fieldset[3]/form/div[3]/div/ul/li[2]')
                    for option in g2.find_elements(By.XPATH,
                                                   '//*[@id="control-bar"]/div/fieldset[3]/form/div[3]/div/ul/li[2]'):
                        option.click()
                if '3 gram' in statement or 'three' in statement:
                    g3 = driver.find_element(By.XPATH,
                                             '//*[@id="control-bar"]/div/fieldset[3]/form/div[3]/div/ul/li[3]')
                    for option in g3.find_elements(By.XPATH,
                                                   '//*[@id="control-bar"]/div/fieldset[3]/form/div[3]/div/ul/li[3]'):
                        option.click()
                if '4 gram' in statement or 'four' in statement:
                    g4 = driver.find_element(By.XPATH,
                                             '//*[@id="control-bar"]/div/fieldset[3]/form/div[3]/div/ul/li[4]')
                    for option in g4.find_elements(By.XPATH,
                                                   '//*[@id="control-bar"]/div/fieldset[3]/form/div[3]/div/ul/li[4]'):
                        option.click()
                if '5 gram' in statement or 'five' in statement:
                    g5 = driver.find_element(By.XPATH,
                                             '//*[@id="control-bar"]/div/fieldset[3]/form/div[3]/div/ul/li[5]')
                    for option in g5.find_elements(By.XPATH,
                                                   '//*[@id="control-bar"]/div/fieldset[3]/form/div[3]/div/ul/li[5]'):
                        option.click()
                if '6 gram' in statement or 'six' in statement:
                    g6 = driver.find_element(By.XPATH,
                                             '//*[@id="control-bar"]/div/fieldset[3]/form/div[3]/div/ul/li[6]')
                    for option in g6.find_elements(By.XPATH,
                                                   '//*[@id="control-bar"]/div/fieldset[3]/form/div[3]/div/ul/li[6]'):
                        option.click()
                if '7 gram' in statement or 'seven' in statement:
                    g7 = driver.find_element(By.XPATH,
                                             '//*[@id="control-bar"]/div/fieldset[3]/form/div[3]/div/ul/li[7]')
                    for option in g7.find_elements(By.XPATH,
                                                   '//*[@id="control-bar"]/div/fieldset[3]/form/div[3]/div/ul/li[7]'):
                        option.click()
                press('enter')
                speak("Here are your results")  # generates the results
                # filters rows
                speak("would you like to filter any rows?")
                statement = takeCommand().lower()
                if 'yes' in statement:
        ...
        # opens keyword tab
            if 'keyword' in statement or 'word' in statement:
                ...
                speak("The Target corpus options are Dickens Novels, 19th Century Reference Corpus, Children's Literature,"
                      "Additional Requested Texts and African American Writers ")
                statement = takeCommand().lower()
                # selects the option according to the statement
                if 'novels' in statement:
                    ...
                    speak("How else may I be of service?")
                    statement = takeCommand().lower()
                # opens the subsets option and selects the required subset according to the statment
                ...
                if 'all text' in statement:
                    ...
                    for option in gre1.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[4]/form/div[2]/div/ul/li[1]'):
                        option.click()
                if 'short suspensions' in statement or 'short' in statement:
                    gre2 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[4]/form/div[2]/div/ul/li[2]')
                    for option in gre2.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[4]/form/div[2]/div/ul/li[2]'):
                        option.click()
                if 'long suspensions' in statement or 'long' in statement:
                    gre3 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[4]/form/div[2]/div/ul/li[3]')
                    for option in gre3.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[4]/form/div[2]/div/ul/li[3]'):
                        option.click()
                if 'quotes' in statement:
                    gre4 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[4]/form/div[2]/div/ul/li[4]')
                    for option in gre4.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[4]/form/div[2]/div/ul/li[4]'):
                        option.click()
                if 'non quotes' in statement or 'non' in statement:
                    gre5 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[4]/form/div[2]/div/ul/li[5]')
                    for option in gre5.find_elements(By.XPATH,
                                                     '//*[@id="control-bar"]/div/fieldset[4]/form/div[2]/div/ul/li[5]'):
                        option.click()
                # selects ngrams
                speak("which ngram would you like to choose?")
                ...
                if '1 gram' in statement or 'one' in statement:
                    ...
                    for option in gr1.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[4]/form/div[3]/div/ul/li[1]'):
                        option.click()
                if '2 gram' in statement or 'two' in statement:
                    gr2 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[4]/form/div[3]/div/ul/li[2]')
                    for option in gr2.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[4]/form/div[3]/div/ul/li[2]'):
                        option.click()
                if '3 gram' in statement or 'three' in statement:
                    gr3 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[4]/form/div[3]/div/ul/li[3]')
                    for option in gr3.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[4]/form/div[3]/div/ul/li[3]'):
                        option.click()
                if '4 gram' in statement or 'four' in statement:
                    gr4 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[4]/form/div[3]/div/ul/li[4]')
                    for option in gr4.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[4]/form/div[3]/div/ul/li[4]'):
                        option.click()
                if '5 gram' in statement or 'five' in statement:
                    gr5 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[4]/form/div[3]/div/ul/li[5]')
                    for option in gr5.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[4]/form/div[3]/div/ul/li[5]'):
                        option.click()
                if '6 gram' in statement or 'six' in statement:
                    gr6 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[4]/form/div[3]/div/ul/li[6]')
                    for option in gr6.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[4]/form/div[3]/div/ul/li[6]'):
                        option.click()
                if '7 gram' in statement or 'seven' in statement:
                    gr7 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[4]/form/div[3]/div/ul/li[7]')
                    for option in gr7.find_elements(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[4]/form/div[3]/div/ul/li[7]'):
                        option.click()
                SEARCH41_CORPORA_BUTTON_XPATH = '//*[@id="control-bar"]/div/fieldset[4]/form/div[4]/ul/li/input'  # clicks on reference corpora search box
                browser = driver
                button1 = browser.find_element(By.XPATH, SEARCH41_CORPORA_BUTTON_XPATH)
                button1.click()
                speak(
                    "The reference corpus options are Dickens Novels, 19th Century Reference Corpus, Children's Literature, "
                    "Additional Requested Texts and African American Writers ")
                statement = takeCommand().lower()
                # selects the option according to voice command
                if 'Dickens Novels' in statement or 'Decans Novels' in statement \
                        or 'novels' in statement:
                    ...
                    speak("How else may I be of service?")
                    statement = takeCommand().lower()
                # selects the subsets options after opening the dropdown for the serach box
                ...
                if 'all text' in statement:
                    ...
                    for option in grea1.find_elements(By.XPATH,
                                                      '//*[@id="control-bar"]/div/fieldset[4]/form/div[5]/div/ul/li[1]'):
                        option.click()
                if 'short suspensions' in statement or 'short' in statement:
                    grea2 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[4]/form/div[5]/div/ul/li[2]')
                    for option in grea2.find_elements(By.XPATH,
                                                      '//*[@id="control-bar"]/div/fieldset[4]/form/div[5]/div/ul/li[2]'):
                        option.click()
                if 'long suspensions' in statement or 'long' in statement:
                    grea3 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[4]/form/div[5]/div/ul/li[3]')
                    for option in grea3.find_elements(By.XPATH,
                                                      '//*[@id="control-bar"]/div/fieldset[4]/form/div[5]/div/ul/li[3]'):
                        option.click()
                if 'quotes' in statement:
                    grea4 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[4]/form/div[5]/div/ul/li[4]')
                    for option in grea4.find_elements(By.XPATH,
                                                      '//*[@id="control-bar"]/div/fieldset[4]/form/div[5]/div/ul/li[4]'):
                        option.click()
                if 'non quotes' in statement or 'non' in statement:
                    grea5 = driver.find_element(By.XPATH, '//*[@id="control-bar"]/div/fieldset[4]/form/div[5]/div/ul/li[5]')
                    for option in grea5.find_elements(By.XPATH,
                                                      '//*[@id="control-bar"]/div/fieldset[4]/form/div[5]/div/ul/li[5]'):
                        option.click()
                speak("Here are your results")  # generates results
                # clicks on button for swapping target and reference corpus
                ...
                if 'yes' in statement:
                    ...
                    button.click()
                    speak("Here are your results")
                # selects p-value cutoff
                speak("would you like to change the p-value cut off?")
                statement = takeCommand().lower()
                if 'yes' in statement:
                    PCO_V = driver.find_element(By.XPATH,
                                                '//*[@id="control-bar"]/div/fieldset[4]/form/div[6]/div/div/input')
                    for option in PCO_V.find_elements(By.XPATH,
                                                      '//*[@id="control-bar"]/div/fieldset[4]/form/div[6]/a/span'):
                        option.click()
                    speak("what value do you want to choose?")
                    if "zero point zero five" in statement:
                        ZPZF = driver.find_element(By.XPATH,
                                                   '//*[@id="control-bar"]/div/fieldset[4]/form/div[6]/div/ul/li[1]')
                        for option in ZPZF.find_elements(By.XPATH,
                                                         '//*[@id="control-bar"]/div/fieldset[4]/form/div[6]/div/ul/li[1]'):
                            option.click()
                    if "zero point zero one" in statement:
                        ZPZO = driver.find_element(By.XPATH,
                                                   '//*[@id="control-bar"]/div/fieldset[4]/form/div[6]/div/ul/li[2]')
                        for option in ZPZO.find_elements(By.XPATH,
                                                         '//*[@id="control-bar"]/div/fieldset[4]/form/div[6]/div/ul/li[2]'):
                            option.click()
                    if "zero point zero zero one" in statement:
                        ZPZZO = driver.find_element(By.XPATH,
                                                    '//*[@id="control-bar"]/div/fieldset[4]/form/div[6]/div/ul/li[3]')
                        for option in ZPZZO.find_elements(By.XPATH,
                                                          '//*[@id="control-bar"]/div/fieldset[4]/form/div[6]/div/ul/li[3]'):
                            option.click()
                else:
                    ZPZZZO = driver.find_element(By.XPATH,
                                                 '//*[@id="control-bar"]/div/fieldset[4]/form/div[6]/div/ul/li[4]')
                    for option in ZPZZZO.find_elements(By.XPATH,
                                                       '//*[@id="control-bar"]/div/fieldset[4]/form/div[6]/div/ul/li[4]'):
                        option.click()
                speak("Here are your results")
                # filters results according to the typed in words
                speak("would you like to filter any rows?")
                statement = takeCommand().lower()
                if 'yes' in statement:
                    speak('please type in your filter terms')
            ...
            if 'counts' in statement or 'count' in statement:
                ...
                if 'Dickens Novels' in statement or 'Decans Novels' in statement \
                    ...
                    speak("How else may I be of service?")
                    statement = takeCommand().lower()
                speak("These are the results of the counts search")  # displays the counts of the books
            # opens texts tab
            if 'texts' in statement or 'text' in statement:
                ...
                statement = takeCommand().lower()
                # selects the options that matches the audio
                if "Bleak House" in statement or "Bleak House" in statement:
                    ...
                    speak("How else may I be of service?")
                    statement = takeCommand().lower()
                    # opens the dropdown for the user and asks the user to select an options manually
                ...
                # selects thee radiio buttons which the user wishes to highlight in the text
                ...
                if 'yes' in statement:
                    ...
                    speak("here are the results")
                # saves the results
                speak("do you wish to save the results?")
                statement = takeCommand().lower()
                if 'yes' in statement:
                    SAVE_BUTTON_XPATH = '//*[@id="control-bar"]/div/div/a[3]'
                    browser = driver
                    button = browser.find_element(By.XPATH, SAVE_BUTTON_XPATH)
                    button.click()
time.sleep(1)
